    ...
    const filteredVerbs = verbs.filter(v => {
        ...
        if (groupFilter !== 'All') {
            if (groupFilter === 'Reflexive') {
                ...
                matchesGroup = v.infinitive.endsWith('re') && !v.infinitive.startsWith('se ') && !v.infinitive.startsWith("s'");
            }
        }
    ...
    return (
        <div className="container" style={{ maxWidth: '1200px', display: 'flex', gap: '2rem', alignItems: 'flex-start' }}>
            ...
            <div style={{
                ...
                position: 'sticky',
                top: '2rem',
                display: 'none', // Hidden on small screens, shown in media query (handled by inline style if simple)
            }} className="top-verbs-sidebar">
                <h3 style={{ textAlign: 'center', marginBottom: '1.5rem', color: 'var(--accent-color)', fontSize: '1.2rem', textTransform: 'uppercase' }}>
                    TOP 20 Most Used French Verbs
                </h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '0.5rem' }}>
                    {topVerbs.map((verbName) => {
                        const verbObj = verbs.find(v => v.infinitive.toLowerCase() === verbName.toLowerCase());
                        return (
                            <button
                                key={verbName}
                                onClick={() => verbObj && setSelectedVerb(verbObj)}
                                style={{
                                    textAlign: 'left',
                                    padding: '0.6rem 1rem',
                                    borderRadius: '8px',
                                    border: '1px solid transparent',
                                    background: selectedVerb?.infinitive.toLowerCase() === verbName.toLowerCase() ? 'rgba(100, 108, 255, 0.2)' : 'transparent',
                                    color: selectedVerb?.infinitive.toLowerCase() === verbName.toLowerCase() ? 'var(--accent-color)' : 'var(--text-primary)',
                                    cursor: 'pointer',
                                    transition: 'all 0.2s',
                                    fontSize: '0.95rem',
                                    display: 'flex',
                                ...
                                onMouseLeave={(e) => {
                                    e.currentTarget.style.background = selectedVerb?.infinitive.toLowerCase() === verbName.toLowerCase() ? 'rgba(100, 108, 255, 0.2)' : 'transparent';
